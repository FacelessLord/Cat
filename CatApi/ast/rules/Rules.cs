    ...
    public class Rules
    {
        ...
        private static CompositeRule ArithmeticExpressionRecursive = Rule.Named(nameof(ArithmeticExpression))
            .With(_ => Chain.StartWith(_).Then(Token(Or)).Then(_)
                .CollectTailBy(nodes => new NodeList(nodes)))
            ...
            .With(_ => Chain.StartWith(Token(LParen)).Then(_).Then(Token(RParen))
                .CollectBy(nodes => ArithmeticTreeCollector(((NodeList) nodes[1]).Nodes)))
            .With(_ => Chain.StartWith(_).Then(Token(Is)).Then(TypeName)
                .CollectTailBy(nodes => new NodeList(nodes)))
            .With(_ => Chain.StartWith(_).Then(Token(As)).Then(TypeName)
                .CollectTailBy(nodes => new NodeList(nodes)))
            .With(_ => Chain.StartWith(_).Then(Token(Dot)).Then(ID)
                .CollectTailBy(nodes => new PropertyAccessNode(ArithmeticTreeCollector(((NodeList) nodes[0]).Nodes), nodes[2] as IdNode)))
            .With(_ => Chain.StartWith(_).Then(Token(LParen)).Then(ExpressionList).Then(Token(RParen))
                .CollectTailBy(nodes => new ObjectCallNode(ArithmeticTreeCollector(((NodeList) nodes[0]).Nodes), nodes[2] as ExpressionListNode)))
            .With(_ => Chain.StartWith(Token(Tilda)).Then(_).CollectBy(nodes => new NodeList(nodes)))
            .With(_ => Chain.StartWith(Token(Plus)).Then(_).CollectBy(nodes => new NodeList(nodes)))
            .With(_ => Chain.StartWith(Token(Minus)).Then(_).CollectBy(nodes => new NodeList(nodes)))
